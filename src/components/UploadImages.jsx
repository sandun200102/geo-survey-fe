...
export default function UploadImages({eqId}) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const name = data.results.map(file => file.Bucket)[0]
        const id = eqId
        const form ={imageKey:imkey,bucketName:name}
        await equipmentStore.updateEquipmentImageKey(id,form)
        setFiles([]);
      } else {
