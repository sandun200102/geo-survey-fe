    ...
    <nav className=" bg-gradient-to-r from-white-400 to-blue-100 backdrop-blur-md shadow-lg w-full top-0 z-50 relative">
      ...
      {user && (
        <button 
          ...
          className="mt-1 transition-all duration-300 transform hover:scale-110 active:scale-95"
        >
          <Avatar fname={user.firstName} lname={user.lastName} size={40} />
          <span className=" text-sm mr-3">{user?.firstName.trim().split(" ")[0]}</span>
        </button>
        )}
        {/* User Menu */}
        ...
        {isUserMenuOpen && (
          <div className="absolute right-0 mt-17 w-48 bg-black/90 backdrop-blur-md rounded-lg shadow-lg p-4 space-y-2">
            <Link to="/admin-dashboard" className="block text-white/80 hover:text-emerald-500 px-3 py-2 rounded-lg group-hover:w-full transition-all duration-300">Profile</Link>
